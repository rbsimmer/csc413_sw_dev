...
public class Lexer {
    ...
    public static void main(String args[]) {
        try {
            ...
            while (tok != null) {       //end loop when no new token read
                String p = "";
                if ((tok.getKind() == Tokens.Identifier)
                        || (tok.getKind() == Tokens.INTeger) || (tok.getKind() == Tokens.FLOAT)) {
                    p += tok.toString();
                } else {
                    p = TokenType.tokens.get(tok.getKind()) + " ";
                }
                String p2 = " left: " + tok.getLeftPosition()
                        + " right: " + tok.getRightPosition();
                System.out.printf("%-15s%s line: %d\n", p, p2, tok.getLineno());
                tok = lex.nextToken();
            }
    ...
    public Token newFloatToken(String number, int startPosition, int endPosition, int lineNum) {
        return new Token(startPosition, endPosition,
                Symbol.symbol(number, Tokens.FLOAT), lineNum);
    }
    ...
    public Token nextToken() { // ch is always the next char to process
        ...
        //catch floating point numbers that begin with a decimal
        if (ch == '.') {
            String num = "";
            try {
                do {
            ...
            if (num.length() > 1) {
                return newFloatToken(num, startPosition, endPosition, source.getLineno());
            } else if (num.length() == 1) {         //Error message for lone decimals
                System.out.println("*****Invalid Floating Point on line "  
                + source.getLineno() + " column " + endPosition + "*****");
                atEOF = true;
                return nextToken();
            }
        }
        //handle floating point integers that start with a digit
